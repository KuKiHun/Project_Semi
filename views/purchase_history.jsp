...
<!--===============================================================================================-->
    ...
    <style>
        h1 {
            ...
            margin-bottom: 50px;
            margin-top: 100px;
            font-family: "Your Font", sans-serif; /* 원하는 폰트로 변경 */
   		    color: #000000; /* 원하는 글자 색상으로 변경 */
   		    text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5); /* 그림자 효과 적용 */
        }
    ...
    <h1>구매내역</h1>
     <table border="1">
        ...
        <c:forEach items="${orderList}" var="order">
         <tr>
                <td>${order.recipientName}</td> <!-- 수취인 -->
                <td>${order.orderedName}</td> <!-- 상품명 -->
                <td>${order.orderedPrice}원</td> <!-- 가격 -->
                <td>${order.orderedAmount}</td> <!-- 수량 -->
                <td>${order.totalPrice}원</td> <!-- 총 결제 금액 -->
                <td>${order.payDate}</td> <!-- 결제일 -->
                <td>${order.orderedState}</td> <!-- 배송 상태 -->
              <td>
                <c:choose>
                    <c:when test="${order.orderedState eq '결제대기중' || order.orderedState eq '결제완료' || order.orderedState eq '배송준비중'}">
                        <a href="/order/cancel/${order.ono}" class="btn btn-dark">취소</a>
                        <button class="btn btn-default btn-xs" disabled>교환 환불</button>
                    </c:when>
                    
                     <c:when test="${order.orderedState eq '취소처리중' || order.orderedState eq '교환처리중' || order.orderedState eq '환불처리중'}">         
                        <button class="btn btn-default btn-xs" disabled>취소 교환 환불</button>
                    </c:when>
                    <c:otherwise>
                        <a href="/order/cancel/${order.ono}" class="btn btn-dark">취소</a>
                        <a href="/order/exchange/${order.ono}" class="btn btn-dark">교환</a>
                        <a href="/order/refund/${order.ono}" class="btn btn-dark">환불</a>
                    </c:otherwise>
                </c:choose>
            </td>
...
<script>
    // 주문 취소 버튼 클릭 시 AJAX 요청 보내기
    function deleteOrder(ono) {
        $.ajax({
            type: 'DELETE',
            url: `/order/cancel/${ono}`,
            success: function (data) {
                alert(data); // 성공 메시지 표시
                location.reload(); // 페이지 새로고침
            },
            error: function (error) {
                alert(error.responseText); // 오류 메시지 표시
            }
        });
    }
</script>
